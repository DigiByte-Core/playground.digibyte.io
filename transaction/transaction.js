...
.controller('TransactionCtrl', function($scope, $rootScope, $http, digibyte) {
  ...
  var reset = function() {
    if (digibyte.Networks.defaultNetwork.name === 'testnet') {
    ...
    $rootScope.transaction = new digibyte.Transaction();
    $scope.privateKey = '';
  ...
  $scope.signWith = function(privKey) {
    try {
      ...
      var privateKey = new digibyte.PrivateKey(privKey);
  ...
  $scope.addAddressOutput = function(address, amount) {
    ...
    amount = digibyte.Unit.fromBTC(amount).toSatoshis();
  ...
  function setExampleCode() {
    ...
    template += "var transaction = new digibyte.Transaction()\n";
  ...
  // Monkey patching until next digibyte version is released
  digibyte.Transaction.prototype.removeInput = function(txId, outputIndex) {
    var index;
