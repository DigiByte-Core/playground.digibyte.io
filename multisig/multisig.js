...
.controller('MultisigCtrl', function($scope, digibyte) {
  ...
  function getRandomKey() {
    var priv = new digibyte.PrivateKey();
    return {
  ...
  $scope.updatePriv = function(index) {
    var privKey = new digibyte.PrivateKey($scope.keys[index].privKey);
  ...
  $scope.updatePub = function(index) {
    ...
    $scope.keys[index].pubKey = new digibyte.PublicKey($scope.keys[index].pubKey).toString();
    setAddress();
  };
  ...
  var setAddress = function() {
    ...
    var address = new digibyte.Address(pubkeys, $scope.threshold);
  ...
  function setExampleCode(pubkeys, threshold) {
    ...
    pubkeys.forEach(function(key, index) {
      template += "  new digibyte.PublicKey('" + key.toString() + "')";
      template += (index < pubkeys.length - 1) ? ',\n' : '\n';
    });

    template += "];\n";
    template += "var requiredSignatures = " + threshold + ";\n";
    template += "var address = new digibyte.Address(publicKeys, requiredSignatures);";
